...
type
  TVertHelper = class helper for TVertScrollBox
     ...
     procedure SkypeCard(txt1,txt2 :String; data :TDate; ID:Integer; Status:Boolean ); overload;
...
procedure TVertHelper.SkypeCard(txt1,txt2 :String; data :TDate; ID:Integer; Status:Boolean );
begin
   ...
   fLayouts.btnExcluir.Tag := ID;
   fLayouts.retSkype.Tag := ID;

   if status = False then
      fLayouts.Circle2.Fill.Color := TAlphaColors.Red;

   fLayouts.Label6.Text := Iniciais(fLayouts.Label4.Text);
   fLayouts.lblDataCadastro.text := FormatDateTime('dd/mm/yyyy',Data);
   //Self.Tag := Self.Tag + 1;
